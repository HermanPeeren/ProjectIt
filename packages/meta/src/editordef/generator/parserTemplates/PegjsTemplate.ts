...
export class PegjsTemplate {
    ...
    listNumber: number = 0;
    // from here
    partClassifiers: PiClassifier[] = [];
    refClassifiers: PiClassifier[] = [];
    // till here

    generatePegjs(language: PiLanguage, editDef: PiEditUnit): string {
...
${this.makeStartRule(language.modelConcept)}
        
${editDef.conceptEditors.map(conceptDef => `${this.makeConceptRule(conceptDef)}`).join("\n")}
${language.interfaces.length > 0 ? `${language.interfaces.map(intf => `${this.makeChoiceRule(intf)}`).join("\n")}` : `` }
...
`;
    ...
    private makeStartRule(modelConcept: PiConcept): string {
        // find all types of the units
        let unitTypes: PiClassifier[] = [];
        modelConcept.parts().forEach(part => {
            const partType = part.type?.referred;
            if (!!partType) {
                unitTypes.push(partType);
            }
        })

        // the rule starts with whitespace in order for comments to be allowed at the start of the file
        return `${Names.classifier(modelConcept)} = ${unitTypes.map((type, index) =>
            `ws var${index}:${Names.classifier(type)} { return var${index}; }`).join("\n    / ")}`;
    }
    ...
    private makeOrdinaryRule(conceptDef: PiEditConcept, piClassifier: PiConcept) {
        ...
        // Note that in the following there is a check on the name of the property.
        // This is because Babel gives a syntax error: "Binding 'arguments' in strict mode (2:6)",
        // when the name is equal to "arguments". According to the info on 10 sep 2020 this is a bug in Babel.
        return `${myName} =${choiceBetweenSubconcepts} ${conceptDef.projection.lines.map(l =>
            `${this.doAllItems(l.items)}`
        ).join("\n\t")}\n\t{ return creator.create${myName}({${propsToSet.map(prop => `${prop.name}:${prop.name === "arguments" ? prop.name + "XX" : prop.name}`).join(", ")}}); }\n`;
    }
